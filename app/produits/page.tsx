...
import { ArrowRight, Bike, Dumbbell, Map, Mountain, Route, ShieldCheck, Sparkles } from "lucide-react";
...
export const metadata: Metadata = publicPageMetadata({
  ...
  description:
    "Découvrez les produits portés par CorsaiManager : Traknio, coach de musculation intelligent, et Sentieru, application de randonnée et d'itinéraires.",
  path: "/produits",
  image: {
    url: "/products/traknio-phone-hero.png",
    width: 1200,
    height: 900,
    alt: "Application mobile Traknio, coach de musculation intelligent",
  },
});
...
const products = [
  {
    ...
    features: [
      ...
      "Synchronisation montre connectée et expérience mobile",
    ],
  ...
  {
    name: "Sentieru",
    label: "Randonnées, itinéraires et préparation terrain",
    href: "https://sentieru.fr",
    image: "/products/sentieru-icon.png",
    logo: "/products/sentieru-icon.png",
    icon: Mountain,
    status: "Application outdoor",
    description:
      "Sentieru aide à découvrir, préparer et suivre ses randonnées avec une approche simple : cartes, itinéraires, GPS et statistiques de marche.",
    features: [
      "Cartes et itinéraires pour préparer la sortie",
      "Suivi GPS pendant la randonnée",
      "Statistiques de marche et historique des parcours",
    ],
    audience: "Randonneurs, familles, clubs et voyageurs qui veulent préparer leurs sorties.",
    accent: "from-emerald-300/20 via-cyan-300/10 to-amber-300/15",
  },
];
      ...
      >
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            ["Mobile", "pensé pour le quotidien"],
          ].map(([value, label]) => (
      ...
      <Container>
        <section className="mt-12 grid gap-6">
          {products.map((product, index) => (
            <article
              key={product.name}
              className={`grid gap-8 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br ${product.accent} p-6 lg:grid-cols-[0.9fr_1.1fr] lg:p-8`}
            >
              <div className={`relative flex min-h-80 items-center justify-center rounded-3xl border border-white/10 bg-zinc-950/70 p-6 ${index % 2 === 1 ? "lg:order-2" : ""}`}>
                {product.name === "Traknio" ? (
                  <Image
                    src={product.image}
                    alt="Interface mobile Traknio avec suivi de musculation"
                    width={900}
                    height={900}
                    className="max-h-[520px] w-auto rounded-3xl object-contain"
                    priority={index === 0}
                  />
                ) : (
                  <div className="grid place-items-center gap-6 text-center">
                    <div className="rounded-[2rem] border border-emerald-300/25 bg-emerald-300/10 p-8 shadow-[0_0_70px_rgba(52,211,153,0.18)]">
                      <Image
                        src={product.image}
                        alt="Icône Sentieru"
                        width={160}
                    ...
                    <div className="grid grid-cols-3 gap-3 text-xs text-zinc-300">
                      {[
                        ...
                        <div key={label as string} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                          <Icon className="mx-auto text-emerald-200" size={22} />
              ...
              <div className="flex flex-col justify-center">
                ...
                {product.name === "Traknio" ? (
                  ...
                  <h2 className="mt-7 text-4xl font-semibold tracking-tight text-zinc-100">{product.name}</h2>
                )}
        ...
        <section className="mt-14 rounded-3xl border border-white/10 bg-zinc-900/60 p-8">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {[
              ["Usage réel", "Chaque produit répond à un usage clair : progresser en sport ou préparer une sortie terrain."],
