...
export async function sendProspectFollowUpEmailAction(formData: FormData) {
  await requireCrmAccess();
  readId(formData, "id");
  readId(formData, "prospectId");

  // CorsaiManager local follow-ups are historical records. New commercial
  // sequences are owned by AI-Team's versioned, policy-checked pipeline.
  // This is a hard server-side boundary, not merely a disabled UI control.
  throw new Error("Relance historique non exécutable : utilisez uniquement une séquence commerciale versionnée active.");
}
